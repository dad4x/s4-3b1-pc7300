 ...
 * - CTIX SV FS with magic in a different place, big or little endian
...
static void detected_ctix(struct sysv_sb_info *sbi, unsigned *max_links)
{
	struct ctix_super_block *sbd;
	struct buffer_head *bh1 = sbi->s_bh1;
	struct buffer_head *bh2 = sbi->s_bh2;

	if (bh1 == bh2)
		sbd = (struct ctix_super_block *) (bh1->b_data + BLOCK_SIZE/2);
	else
		sbd = (struct ctix_super_block *) bh2->b_data;
	...
	*max_links = SYSV_LINK_MAX;
	...
	sbi->s_free_blocks = &sbd->s_tfree;
	sbi->s_sb_time = &sbd->s_time;
	sbi->s_sb_state = 0;	/* no state in CTIX sb */
	sbi->s_firstdatazone = fs16_to_cpu(sbi, sbd->s_isize);
	sbi->s_nzones = fs32_to_cpu(sbi, sbd->s_fsize);
}
...
static int detect_ctix(struct sysv_sb_info *sbi, struct buffer_head *bh)
{
	struct ctix_super_block * sbd;
	u32 type;
...
static int sysv_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	BUILD_BUG_ON(512 != sizeof (struct ctix_super_block));
